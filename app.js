...
function DetalleHome()
{
	...
	const totalPelis = 'Total de películas en cartelera​: (' + movies.length +')';
	
	//c. Listados de películas 
	const listadoPeli = '\n\n\n Listados de películas:'
	const pelis = movies.map(movies => movies.title);
	
	//Mostrar el listado de películas organizadas alfabéticamente por  título.  
	pelis.sort();
	let peliListas = '\n';
	...
	//Pié de página​: ​Recordá que podés visitar las secciones:  
...
function cargarDetalleVotadas(){
	let titulo = 'Más Votadas\n\n';
	
	let soloVoto= movies.map(movies => movies.vote_average);
    let mayores = soloVoto.filter(function(voto){
		return voto >= 7;})
	let promedioMayor = 'Total de películas :' + mayores.length+ '\n\n';


	let sumaVotos = mayores.reduce(function(acum, num){
        return (acum + num)
	});
	let votoPromedio = 'Rating promedio: ' + Math.round (sumaVotos/mayores.length) + '\n\n';

        
	let mejores = movies.filter(movie => movie.vote_average >= 7);
	let masVotadas = ' \n';
	for (let pelis of mejores) {
	masVotadas += pelis.title+' \n'+ pelis.vote_average+' \n'+ pelis.overview+' \n\n';
}
return titulo + promedioMayor + votoPromedio + masVotadas;
}

function cargarDetalleSucursales(){
	let titulo = 'Nuestras Salas';
	let totalSalas = '\n\nTotal de salas: ' + theaters.length + '\n\n'
		let datosTeatros = ''
		theaters.forEach(elemento => datosTeatros += elemento.name + '\n' + elemento.address + '\n' + elemento.description + '\n\n')

return titulo + totalSalas + datosTeatros;
}

function cargarDetalleContacto(){
	let titulo= 'Contáctanos \n\n';
	let cuerpo= '¿Tenés alto para contarnos? \n Nos encanta escuchar a nuestros clientes. Si deseas contactarnos podés escribirnos al siguiente mail: \n dhmovies@digitalhouse.com o en las redes sociales.\n\n Envianos tu consulta, sugerencia o reclamo y será respondido a la brevedad posible. \n\n Recordá que también podes consultar la sección de Preguntas Frecuentes para obtener respuestas inmediatas a los problemas más comunes.';

	return titulo + cuerpo;

}

function cargarDetalleFaqs(){
...
http.createServer((req, res)=> {
	...
	switch (req.url) {
		...
		case '/sucursales':
			...
			case '/contacto':
			let contacto = cargarDetalleContacto ();
			res.end(contacto);
			break;
		
		case '/preguntas-frecuentes':
			let faqs = cargarDetalleFaqs();
			res.end(faqs);
			break;
